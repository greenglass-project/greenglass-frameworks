...
class DriversService(private val gpio : GpioService,
    ...
    fun subscribe(): SharedFlow<List<MetricNameValue>> = publishMetricsFlow.asSharedFlow()
    ...
    suspend fun registerDrivers(definition: NodeDefinition) {
        ...
        definition.metrics.forEach { m ->
            logger.debug { "Metric ${m.metricName} -> (${m.driver}, ${m.function})" }
        ...
        webService.addGet("/drivers") { ctx ->
            ctx.json(Json.encodeToString(driversList))
        }
    }
