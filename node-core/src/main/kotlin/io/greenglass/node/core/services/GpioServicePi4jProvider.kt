...
class GpioServicePi4jProvider {
    ...
    inner class I2cController(bus : Int, device : Int) {
        ...
        val dev : I2C
        var i2CProvider = pi4j.provider<I2CProvider>("linuxfs-i2c")

        init {
            val config: I2CConfig = I2C
                .newConfigBuilder(pi4j)
    ...
    companion object {
        @Volatile
        private var instance: GpioServicePi4jProvider? = null

        val gpio: GpioServicePi4jProvider
            get() {
                if (instance == null) {
                    synchronized(this) {
                        if (instance == null) {
                            instance = GpioServicePi4jProvider()
                        }
                    }
                }
                return instance!!
